...
func initDb() {
	...
	hotPostsOut, err = db.Prepare("SELECT pid, email_hash, text, timestamp, tag, type, file_path, likenum, replynum FROM posts WHERE pid>(SELECT MAX(pid)-1000 FROM posts) AND reportnum<10 ORDER BY likenum*3+replynum+(likenum/(replynum+1))*50-reportnum*20 DESC")
	fatalErrorHandle(&err, "error preparing posts sql query")
